...
TRES_OUTPUT_PATH = "GuildForge/data/defs"
...
def install_dependencies():
    """Installe les dépendances Python nécessaires (xmltodict)."""
    print("Installation des dépendances Python...")
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "xmltodict"])
        print("xmltodict installé avec succès.")
    except subprocess.CalledProcessError as e:
        print(f"Erreur lors de l'installation de xmltodict: {e}")
        sys.exit(1)
...
def generate_gdscript_class(def_type, is_comp_property=False):
    ...
    file_path = os.path.join(GD_SCRIPT_OUTPUT_PATH, f"{class_name}.gd")
    ...
    if class_name == "CR_Base" or class_name == "CR_CompProperties":
        return
...
def generate_tres_instance(def_type, def_name, properties):
    """Génère le fichier .tres (instance de Custom Resource)."""
    class_name = f"CR_{def_type}"
    file_path = os.path.join(TRES_OUTPUT_PATH, f"{def_name}.tres")

    # 1. Déterminer le chemin de la classe GDScript pour ExtResource
    gd_script_path = f"{GD_SCRIPT_PROJECT_PATH}{class_name}.gd"
    ...
    def serialize_value(val):
        ...
        if isinstance(val, dict):
            ...
            if "@Class" in val:
                ...
                script_path = f"res://{GD_SCRIPT_OUTPUT_PATH}/CR_{comp_class}.gd"
        ...
        if key == "comps" and isinstance(value, dict) and "li" in value:
            ...
            for comp in comp_list:
                if isinstance(comp, dict) and "@Class" in comp:
                    ...
                    # Logique de sérialisation de SubResource à implémenter dans generate_tres_instance
                        ...
                        for def_type, def_list in defs_container.items():
                            ...
                            for def_data in def_list:
                                ...
                                if def_name != "UnknownDef":
                                    # Le chemin final sera res://GuildForge/data/defs/DefName.tres
                                    tres_path = f"{PROJECT_ROOT}{TRES_OUTPUT_PATH}/{def_name}.tres"
...
def run_pipeline():
    ...
    # install_dependencies() # Commenté car fait manuellement avec sudo
    
    # 2. Création des répertoires
    create_output_dirs()
    
    # 3. Pré-traitement (P10 Partiel)
    collect_all_def_names()
    
    # 4. Parcours des fichiers XML (Seconde passe pour la conversion)
                    ...
                    if "Defs" in parsed_xml:
                        ...
                        for def_type, def_list in defs_container.items():
                            ...
                            for def_data in def_list:
                                ...
                                generate_tres_instance(def_type, def_name, properties)
